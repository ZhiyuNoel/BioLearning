...
from PyQt6.QtCore import QBasicTimer, QThread, pyqtSignal, pyqtSlot, QEventLoop
from PyQt6.QtGui import QFont
from PyQt6.QtWidgets import QApplication
from torch import nn, optim

from .Pages import Pages
from ..model import select_device, LSTMAutoencoder
...
class Ui_encoder(Pages):
    ...
    def __init__(self, encoPage):
        ...
        self.stopButton = QtWidgets.QPushButton(parent=self.verticalLayoutWidget)
        self.stopButton.setObjectName("pushButton_4")
        self.stopButton.setSizePolicy(buttonSizePolicy)
    ...
    def retranslateUi(self, Form):
        ...
        self.trainButton.clicked.connect(lambda: self.start_train_pipeline())
        self.stopButton.clicked.connect(lambda: self.pause_training())
    ...
    def confirm_parameter(self):
        ...
        self.trainLoader, self.testLoader = loader_pipeline(**loader_kargs)
        self.textBrowser.setText("Data Load Complete")
        self.training_thread = TrainingThread(self.trainLoader, self.start_lr)
        self.processBar.setMaximum(len(self.trainLoader))
...
class TrainingThread(QThread):
    ...
    def __init__(self, train_loader, start_lr):
        super().__init__()
        self.train_loader = train_loader
        self.is_running = True
        self.paused = False
        self.device = select_device("mps")
        self.Autoencoder = LSTMAutoencoder(self.device)
        self.criterion = nn.MSELoss()
        self.optimizer = optim.Adam(self.Autoencoder.parameters(), lr=start_lr)
    ...
    def run(self):
        while True:
            for step, (frames, labels) in enumerate(self.train_loader):
                ...
                outputs = self.Autoencoder(frames)
                loss = self.criterion(outputs, frames)
                loss.backward()
                # 更新参数
                self.optimizer.step()
                self.update_progress.emit(step)  # 发送进度信号
                self.update_text.emit(f"Step {step}: Loss: {loss}")  # 发送文本更新信号
