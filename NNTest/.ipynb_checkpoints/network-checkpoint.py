...
class Model(nn.Module):
    ...
    def encode(self, x, width, height):    #Edit this method without changing the intup and output tensor dimentions.
        #batch_size, _ = x.shape
        #x = x.view(batch_size, 1, height, width)
        #x = torch.tanh(self.EncodeConv0(x))
        #x = torch.tanh(self.EncodeConv1(x))
        #x = torch.tanh(self.EncodeConv2(x))
        #x = torch.tanh(self.EncodeConv3(x))
        #x = torch.tanh(self.EncodeConv4(x))
    ...
    def decode(self, x, width, height):    #Edit this method without changing the intup and output tensor dimentions.
    ...
    串联编码与解码过程，形成自编译器
    '''
    def autoencoder_train(self, x, width, height):
        x = self.encode(x, width, height)
        x = self.decode(x, width, height)
        return x
